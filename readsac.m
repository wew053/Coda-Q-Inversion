...
% if only one file is specified. [INDEP, DEP1, DEP2] = READSAC(FILENAMES); 
% reads spectral type of SAC data files and
% [INDEP1, INDEP2, DEP] = READSAC(FILENAMES); reads xyz type of SAC data files.
%
...
%
% If the SAC data file is not evenly-spaced, time-series file, following fields
% may exist.
%
...
for f = 1:nfiles
   ...
   if fid ~= -1
      ...
      if count1 == 70 && count2 == 40 && count3 == 192 && ...
         B(10) >= 1 && ismember(B(16), [1:4 51]) && ismember(B(36), [0 1])  
         ...
         if strcmp(s(f).IFTYPE,'IXYZ')
            [D, count3] = fread(fid, [s(f).NPTS,1], 'float32');
            s(f).DATA3 = D;
         end
...
% type of SAC data files or xyz data from xyz SAC data files. In case of 
% spectral files, OUT1 is frequency, OUT2 is either real part or amplitude,
% and OUT3 is either imaginary part or phase.
%
% S is defined in readsac.m and is usually the output from readsac.m.
